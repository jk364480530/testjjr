                    ...
                    <table class="layui-table">
                        ...
                        <?php if(!empty($list) && is_array($list)):?>
                            <?php foreach ($list as $key=>$val):?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php elseif($val['status']==4) :?>
                                            <a href="javascript:void(0);" onclick="lailu_detail(<?=$val['id']?>);" style="color:#009e94">[搜索来路]</a>
                                        <?php else:?>
                                    ...
                                    <td>
                                        <?php if($val['status']==2):?>
                                            <button class="layui-btn layui-btn-warm layui-btn-mini" onclick="bind_buyer(<?=$val['id']?>,<?=$val['is_verify']?>);">绑定买号</button>
                                            <br>
                                        ...
                                        <?php elseif($val['status']==3):?>
                                            <button class="layui-btn layui-btn-warm layui-btn-mini layui-btn-disabled">等待审核</button>
                                            <br>
                                            <a href="javascript:void(0);" id="cancel_task<?=$val['id']?>" buy_account_id="<?=$val['buy_account_id']?>" onclick="cancel_task(<?=$val['id']?>);" style="color:#009688">取消任务</a>
                                            <script>
                                                var js<?=$val['id']?>=setInterval("timer(<?=$val['count_down']?>,<?=$val['id']?>,js<?=$val['id']?>)",1000);
                                        ...
                                        <?php elseif($val['status']==4):?>
                                            <button class="layui-btn layui-btn-warm layui-btn-mini" onclick="lailu_detail(<?=$val['id']?>);">搜索验证</button>
                                            <br>
                                            <a href="javascript:void(0);" id="cancel_task<?=$val['id']?>" buy_account_id="<?=$val['buy_account_id']?>" onclick="cancel_task(<?=$val['id']?>);" style="color:#009688">取消任务</a>
                                            <script>
                                                var js<?=$val['id']?>=setInterval("timer(<?=$val['count_down']?>,<?=$val['id']?>,js<?=$val['id']?>)",1000);
                                        ...
                                        <?php elseif($val['status']==5):?>
                                            <button class="layui-btn layui-btn-warm layui-btn-mini" onclick="payment(<?=$val['id']?>);">确认支付</button>
                                            <br>
                                            <a href="javascript:void(0);" id="cancel_task<?=$val['id']?>" buy_account_id="<?=$val['buy_account_id']?>" onclick="cancel_task(<?=$val['id']?>);" style="color:#009688">取消任务</a>
                                            <script>
                                                var js<?=$val['id']?>=setInterval("timer(<?=$val['count_down']?>,<?=$val['id']?>,js<?=$val['id']?>)",1000);
                                        ...
                                        <?php elseif($val['status']==6):?>
                                            <button class="layui-btn layui-btn-warm layui-btn-mini layui-btn-disabled">等待发货</button>
                                        ...
                                        <?php elseif($val['status']==8):?>
                                            <button class="layui-btn layui-btn-warm layui-btn-mini" onclick="praise(<?=$val['id']?>)">好评凭证</button>
                                        <?php elseif($val['status']==9):?>
                                            <button class="layui-btn layui-btn-warm layui-btn-mini layui-btn-disabled">等待放款</button>
                                ...
                                <tr>
                                    <td colspan="6">
                                        <img src="<?=config_item('domain_static')?>home/images/task/seller_bg.png" alt="">&nbsp;&nbsp;<span style="font-weight: bold;">详情:</span>
                                        <?=$val['release_username']?>
                                        <a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=<?=$val['release_qq']?>&site=qq&menu=yes">
...
<script src="<?=config_item('domain_static')?>home/js/my_accept.js"></script>
